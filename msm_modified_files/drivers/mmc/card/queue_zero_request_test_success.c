...
#ifdef DZ_DEBUG
int record_start_req(struct request* req, struct record_structure* record){
    mutex_lock(&(record->record_lock));
    if(req){
        record->rq_start.tv_sec = req->rq_start.tv_sec;
        record->rq_start.tv_nsec = req->rq_start.tv_nsec;
        record->rq_io_start.tv_sec = req->rq_io_start.tv_sec;
        record->rq_io_start.tv_nsec = req->rq_io_start.tv_nsec;
        record->start_time_rq=(unsigned long)req->start_time;
        record->start_time_ns =rq_start_time_ns(req);
        record->io_start_time_ns =rq_io_start_time_ns(req);
        record->rq_size_create = req->rq_size_create;
        record->rq_size_io_start = req->rq_size_io_start;
    }
    getnstimeofday(&record->ts_start);
    return 0;
}
int record_end_req(struct request* req,struct record_structure* record){
 if( req && record->log_buff){
        record->return_time = jiffies;
        record->return_time_ns=sched_clock();
        getnstimeofday(&record->ts_end);
        if(record->rq_size_create==0)
            printk("the request size is zero when created ####################\n");
            else if(record->rq_size_io_start==0)
                printk("the request size is zero when sent to io******************\n");
            if(record->output_len<= record->buffer_sz - 512){
                record->output_len += sprintf(record->log_buff+record->output_len,"%lu\t\t%d\t\t%d\t\t%d\t\t%lld.%.9ld\t\t%lld.%.9ld\t\t%lld.%.9ld\t\t%lld.%.9ld\n",\
                    (unsigned long)blk_rq_pos(req),\
                    (int)blk_rq_sectors(req) + record->total_sectors,\
                    (int) blk_rq_bytes(req),\
                    (int)rq_data_dir(req),\
                    (long long)record->rq_start.tv_sec,\
                    record->rq_start.tv_nsec,\
                    (long long)record->rq_io_start.tv_sec,\
                    record->rq_io_start.tv_nsec,\
                    (long long)record->ts_start.tv_sec,\
                    record->ts_start.tv_nsec,\
                    (long long)record->ts_end.tv_sec,\
                    record->ts_end.tv_nsec);
            }
            else{
...
void record_init(struct record_structure* record){
    ...
    record->output_len=0;
    record->pg_num_order =0;
    record->total_sectors=0;
    record->flags = O_RDWR|O_CREAT|O_APPEND;
    record->rights= 644;
    record->offset = 0;
    record->rq_size_create=0;
    record->rq_size_io_start=0;
    record->start_time_rq =0;
    record->return_time =0;
    record->start_time_ns =0;
    record->return_time_ns =0;
    record->io_start_time_ns =0;
