...
/// added by dzhou
    if( req!=NULL && (&brq->mrq != NULL) && mq){
        brq->mrq.rt.record = &mq->record;
        brq->mrq.rt.record->record_collect(req,&brq->mrq.rt,blk_rq_sectors(req), mq);
    }
}
...
static u8 mmc_blk_prep_packed_list(struct mmc_queue *mq, struct request *req)
{
	...
	while (reqs < max_packed_rw - 1) {
		...
		reqs++;
		/// added by dzhou
  ///      mq->record.total_sectors += blk_rq_sectors(cur);
	}
					...
					struct mmc_queue *mq)
{
    ...
    if( req!=NULL && (&brq->mrq != NULL) && mq){
        brq->mrq.rt.record = &mq->record;
        brq->mrq.rt.record->record_collect(req,&brq->mrq.rt,brq->data.blocks, mq);
    }
}
