...
static void mmc_wait_data_done(struct mmc_request *mrq)
{
	...
	/// added by dzhou
	if(mrq != NULL && mrq->rt.record !=NULL){;
	        mrq->rt.record->record_end(mrq->rt.record, &mrq->rt);
    }
    else
    {
        if (mrq)
            printk("record is null in function: %s *******************\n",__FUNCTION__);
            else
                printk("mrq is null in function: %s *******************\n",__FUNCTION__);
    }
}
...
static int __mmc_start_data_req(struct mmc_host *host, struct mmc_request *mrq)
{
	...
	if (mmc_card_removed(host->card)) {
		...
		/// added by dzhou
	if(mrq != NULL && mrq->rt.record !=NULL){;
        mrq->rt.record->record_start(mrq->rt.record, &mrq->rt);
    }
    else
    {
        if (mrq)
		 ...
		 bool is_first_req)
{
	if (host->ops->pre_req) {
		...
		/// record if this request is first request or not ant the third bit of the access_dir.
		if (mrq && is_first_req){
            mrq->rt.access_dir = mrq->rt.access_dir | 4;
		}
		mmc_host_clk_release(host);
	}
}
				    ...
				    struct mmc_async_req *areq, int *error)
{
	...
	if (host->areq) {
		...
		if (err == MMC_BLK_URGENT || err == MMC_BLK_URGENT_DONE) {
			...
			host->areq = NULL;
			if (areq) {
			/// added by dzhou, if new request is not urgent request
			/// put back to the request list.
			/// else proceed this request.
