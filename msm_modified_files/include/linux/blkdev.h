...
#ifdef IO_TIMING
static inline void set_rq_start_time(struct request *req)
{
	getnstimeofday(&(req->rq_start));
	/// printk("%lld    %ld&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&\n",(long long)req->rq_start.tv_sec, req->rq_start.tv_nsec);
}
static inline void set_rq_io_start_time(struct request *req)
{
	getnstimeofday(&(req->rq_io_start));
}
#else
...
#else
static inline void set_start_time_ns(struct request *req) {set_rq_start_time(req);}
static inline void set_io_start_time_ns(struct request *req) {set_rq_io_start_time(req);}
static inline uint64_t rq_start_time_ns(struct request *req)
{
	return 0;
}
